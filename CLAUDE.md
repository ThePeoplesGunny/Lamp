...
- **(c) Second translation** (e.g. ASV 1901, PD) for translation-drift side-by-side. Now that the KJV↔Heb versification map is solid, a second English translation can reuse the same `versification_kjv_to_heb.json` if it follows KJV numbering.
- **(d) Fix pre-existing GenealogyTree TS errors** — unblocks `npm run build`; small cleanup.
- **(e) Browser-visual QA pass** of Phase 2D-4 + everything built so far (see "Known gaps" for specific URLs).
- **(f) Safer graph-rebuild path** — `seed_graph.py` currently wipes verse SQLite via FK cascade. Could either make it additive (`.load()` + entity-only replace) or add a `reseed_all.sh` that runs the full 6-script chain.
